...
给定一个长为n的数组。有一个大小为 k 的滑动窗口，它从数组的最左边移动到最右边。你只能在窗口中看到 k 个数字。每次滑动窗口向右移动一个位置。你的任务是确定滑动窗口位于每个位置时，窗口中的最大值和最小值。
...
int main(){
    ...
    for(int i = 0; i < n; i++){
        if(hh <= tt && i - k + 1 > q[hh]) hh++;// 如果队列不为空且当前滑动窗口已经滑动过头部元素的索引，则将头部指针 hh 向右移动。
