...
思路：以最小值为例，不断将索引加入窗口队列，如果窗口溢出，就弹出队头；若当前索引对应的元素小于队尾元素，就不断弹出队尾元素。将当前索引加入队列。如果窗口已经滑动过前k个元素，就可以输出队头，即为最小值。

下载virtual box

在virtual box里打开kali

在kali里新建虚拟机，类型：Linux，版本：ubutun

使用已有的虚拟盘-注册-在刚刚下载的目录里选中vdi文件-选择vdi文件

在工具栏里选：管理-工具-网络管理器，点击NAT网络标签，再点击创建，点击右下角的应用

点击刚创建的虚拟机，选择设置，点击常规-高级，共享粘贴板：双向；常规-系统，调整内存分配；常规-网络-连接方式：选择NAT网络；常规-网络-混杂模式：全部允许，点击确定
